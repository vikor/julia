...
// (use in a conditional breakpoint)
int jl_arg_is_typetype_oftype(jl_value_t *args, int indx, char* typename)
{
    if (!jl_is_tuple_type(args))
        return 0;
    jl_datatype_t *ta = (jl_datatype_t*)args;
    if (jl_nparams(ta) <= indx)
        return 0;
    jl_value_t *p = jl_tparam(ta, indx);
    if (!jl_is_datatype(p))
        return 0;
    jl_datatype_t *tp = (jl_datatype_t*)p;
    if (tp->name != jl_type_type->name)
        return 0;
    jl_value_t *T = jl_tparam0(tp);
    if (!jl_is_datatype(T))
        return 0;
    return ((jl_datatype_t*)T)->name->name == jl_symbol(typename);
}
